...
export default class App extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      addressBech32SendADA:
        "addr_test1qq4k0vla0uaanv8nmy5fwrny69p39qfulp65mj2emv7n0pzqfqyh837cs0qkgaj8ryezsmtgh3dkyltl0qnjkj4z5hkqk00ycd",
  ...
  generateScriptAddress = () => {
    ...
    console.log(Buffer.from(ScriptAddress.to_bytes()).toString("hex"));
  ...
  buildSendADATransaction = async () => {
    try {
        ...
        txBuilder.add_inputs_from(txUnspentOutputs, 1); // use 1 for RandomImprove
        ...
        const submittedTxHash = await this.API.submitTx(Buffer.from(signedTx.to_bytes()).toString("hex"));
        console.log(submittedTxHash);
        this.setState({submittedTxHash});
    } catch (e) {
        alert("Send ADA transaction failed: " + (e.message || e));
        console.error(e);
    }
};
  buildSendTokenTransaction = async () => {
    // We need to import the CSL library dynamically for web environments
    const {
      Address,
      TransactionOutputBuilder,
      MultiAsset,
    ...
    try {
      ...
      multiAsset.insert(
        ScriptHash.from_bytes(Buffer.from(this.state.assetPolicyIdHex, "hex")),
        assets
      );

      // FIX: The parameter from Blockfrost is `coins_per_utxo_word`. Ensure your `this.protocolParams` reflects this.
      ...
      const txUnspentOutputs = await this.getTxUnspentOutputs();
      // CRITICAL FIX: Changed coin selection strategy from 3 to 2.
      // Strategy 3 is for spending assets, not for gathering ADA.
      // Strategy 2 (RandomImprove) is the correct choice here.
      txBuilder.add_inputs_from(txUnspentOutputs, 2);

      txBuilder.add_change_if_needed(shelleyChangeAddress);
      const txBody = txBuilder.build();

      // --- Identical Simplified Signing Process ---

      const transaction = Transaction.new(txBody, TransactionWitnessSet.new());
      const txVkeyWitnessesHex = await this.API.signTx(
        Buffer.from(transaction.to_bytes()).toString("hex"),
        true
      );
      const newWitnessSet = TransactionWitnessSet.from_bytes(
        Buffer.from(txVkeyWitnessesHex, "hex")
      );
      const signedTx = Transaction.new(transaction.body(), newWitnessSet);
  ...
  buildRedeemAdaFromPlutusScript = async () => {
    ...
    txBuilder.add_input(
      ...
      TransactionInput.new(
        ...
        this.state.transactionIndxLocked
      ),
